  ...
  componentDidMount() {
    ...
    query.on("value", snapshot => {
      ...
      snapshot.forEach(childSnapshot => {
        ...
        isToday
          ? (todayCounter =
              todayCounter + parseInt(childSnapshot.child("totalPrice").val()))
          : (todayCounter = todayCounter + 0);

        isWeek
          ? (weekCounter =
              weekCounter + parseInt(childSnapshot.child("totalPrice").val()))
        ...
        cummulativeCounter =
          cummulativeCounter +
          parseInt(childSnapshot.child("totalPrice").val());
      });
